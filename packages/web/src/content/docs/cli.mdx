...
Use `--output-schema` to request schema-constrained JSON output from the model. In the default format, stdout contains only the final structured JSON object.

```bash
opencode run --output-schema ./schema.json "Extract the invoice fields"
opencode run --output-schema '{"type":"object","properties":{"answer":{"type":"string"}},"required":["answer"]}' "Answer briefly"
```

`--format json` prints the raw line-delimited CLI event stream. `--output-schema` asks the model to return a schema-constrained final value; combining both keeps the event stream and adds a final structured event.
...
| <nobr><code>{"--output-schema"}</code></nobr> |       | JSON schema file path or inline JSON schema for structured output          |
